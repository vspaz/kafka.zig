...
pub const Producer = struct {
    ...
    pub fn createKafkaProducer(kafka_conf: ?*librdkafka.struct_rd_kafka_conf_s) ?*librdkafka.rd_kafka_t {
        var err_message: [512]u8 = undefined;
    ...
    pub fn init(kafka_conf: ?*librdkafka.struct_rd_kafka_conf_s, topic_conf: ?*librdkafka.struct_rd_kafka_topic_conf_s, topic_name: [*]const u8) Self {
        const kafka_producer: ?*librdkafka.rd_kafka_t = createKafkaProducer(kafka_conf);
    ...
    pub fn deinit(self: Self) void {
        const err_code: c_int = librdkafka.rd_kafka_flush(self._producer, 60_000);
        if (err_code != librdkafka.RD_KAFKA_RESP_ERR_NO_ERROR) {
            @branchHint(.unlikely);
            std.log.err("failed to flush messages {s}", .{errors.err2Str(err_code)});
        }
    ...
    pub fn send(self: Self, message: []const u8, key: []const u8) void {
        ...
        const err_code: c_int = librdkafka.rd_kafka_produce(
            self._topic,
    ...
    // not really needed as 'rd_kafka_flush' is triggered on 'deinit'.
    ...
    pub inline fn init_transactions(self: Self, comptime timeout_ms: u16) i32 {
        ...
        const err_code = errors.err2code(err);
        if (err_code == librdkafka.RD_KAFKA_RESP_ERR_NO_ERROR) {
            std.log.info("Transactions initialized successfully!", .{});
        } else {
            std.log.err("Failed to initialize transactions: {s}", .{errors.err2Str(err_code)});
        }
        errors.deinitErr(err);
        return err_code;
    }
    ...
    pub inline fn begin_transaction(self: Self) i32 {
        ...
        if (err_code == librdkafka.RD_KAFKA_RESP_ERR_NO_ERROR) {
            ...
            std.log.err("Failed to begin transaction: {s}", .{errors.err2Str(err_code)});
        }
        errors.deinit(err);
        return err_code;
    }
    ...
    pub inline fn commit_transaction(self: Self) i32 {
        ...
        if (err_code == librdkafka.RD_KAFKA_RESP_ERR_NO_ERROR) {
            ...
            std.log.err("Failed to commit transaction: {s}", .{errors.err2Str(err_code)});
        }
        errors.deinit(err);
        return err_code;
    }
    ...
    pub inline fn abort_transaction(self: Self, timeout_ms: u16) i32 {
        ...
        if (err_code == librdkafka.RD_KAFKA_RESP_ERR_NO_ERROR) {
            ...
            std.log.err("Failed to abord transaction: {s}", errors.err2Str(err_code));
        }
        errors.deinit(err);
        return err_code;
    }
